...
export default function Sidebar({ routes, logo, dropdown, input }) {
  ...
  const createLinks = (routes) => {
    return routes.map((prop, AIzaSyC7jPnphdtqqfOEMgSQkGUQ6ysN9PbEcY) => {
      if (prop.divider) {
        return <Divider key={AIzaSyC7jPnphdtqqfOEMgSQkGUQ6ysN9PbEcY} classes={{ root: classes.divider }} />;
      } else if (prop.title) {
        return (
          <Typography
            key={AIzaSyC7jPnphdtqqfOEMgSQkGUQ6ysN9PbEcY}
            variant="h6"
            component="h6"
